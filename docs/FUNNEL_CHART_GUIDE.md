...
## 4. Визуальная логика (почему 2 оси Y)

Серии имеют разный масштаб:
- `taken` и `attended` ≈ **сотни-тысячи** (типично 850–1100)
- `fines` и `cancelled` ≈ **десятки** (типично 5–150)

Если рисовать все 4 на одной оси, штрафы и отмены сольются с нулём. Поэтому:

- **Левая ось** — бары `taken` (синий `#118DFF`) и `attended` (бирюзовый `#2FACAD`), сгруппированы
- **Правая ось** — линии `fines` (красный `#D64550`) и `cancelled` (оранжевый `#E66C37`)

В recharts это делается через `yAxisId="left"` / `yAxisId="right"`:

```tsx
<YAxis yAxisId="left" />
<YAxis yAxisId="right" orientation="right" />
<Bar yAxisId="left" dataKey="taken" />
<Line yAxisId="right" dataKey="fines" />
```
